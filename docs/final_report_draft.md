...
## 3. System Overview
The system accepts a short text message and an uploaded facial image, analyzes both modalities, combines the resulting probability distributions, and generates a short empathetic response. The text path is transformer-first: it attempts a pretrained RoBERTa-based emotion classifier before using a deterministic lexical detector only when loading or inference fails. The face path attempts image-based facial inference at runtime, uses DeepFace when available, and falls back to a safe neutral-leaning distribution only when the model or face detection/inference step fails. The response path is FLAN-T5-first: it conditions on the user utterance and the fused emotional state to produce a concise, empathetic reply, and it falls back to a safe template response only when loading or generation fails.
...
The text emotion module predicts probabilities over seven emotions: happy, sad, angry, neutral, fear, surprise, and disgust. The transformer runtime is the primary path, and its broader labels are mapped into the project schema before being normalized to a probability distribution. Labels such as joy, love, optimism, gratitude, approval, curiosity, and sadness are projected into the seven-class output so that the system remains aligned with the project's FER-style schema. If transformer loading or inference fails, the module falls back to a rule-based scoring method that produces the same seven-class output for failure recovery only.

The face module follows the same principle of graceful degradation. When an image is present, the system first attempts image-based facial inference and maps the resulting scores into the same FER-style seven-emotion schema used by the text module: angry, disgust, fear, happy, sad, surprise, and neutral. When DeepFace or face detection cannot complete the analysis, the module returns a safe fallback distribution that is mostly neutral and preserves the same seven-class interface. This ensures that the rest of the prototype can continue operating even in a constrained demo environment.

The response generator follows the same FLAN-T5-first design. It takes the user utterance and the fused emotional state as inputs, then synthesizes a short response that is supportive, socially aware, and aligned with the detected context. If the model cannot be loaded or generation fails for a specific turn, the system returns a concise template-based fallback response so the demo remains usable without changing the interaction flow.
